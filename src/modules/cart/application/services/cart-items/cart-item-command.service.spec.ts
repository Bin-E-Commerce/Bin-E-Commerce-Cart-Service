...
} from '@/modules/cart/application/errors/cart-item.errors';
import { CartOwnerType } from '@/modules/cart/application/types/cart-identity.type';
import type { ProductCatalogProduct } from '@/modules/cart/application/types/product-catalog-product.type';
import { AddCartItemDto } from '@/modules/cart/presentation/dto/add-cart-item.dto';
...
describe('CartItemCommandService', () => {
    ...
    function product(
        ...
        return {
            ...
            name: 'Áo thể thao',
            images: [
                { imageUrl: 'https://cdn.test/product.png', isThumbnail: true },
            ],
            variants: [
                {
                    id: 'variant-1',
    ...
    it('updates quantity when the requested quantity is available', async () => {
        ...
        // Act
        const result = await target.updateItem(identity, 'item-1', {
            quantity: 4,
        });
