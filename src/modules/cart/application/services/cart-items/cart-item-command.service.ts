...
import {
    CartIdentity,
    CartOwnerType,
} from '@/modules/cart/application/types/cart-identity.type';
import { CartResponse } from '@/modules/cart/application/types/cart-response.type';
import { AddCartItemDto } from '@/modules/cart/presentation/dto/add-cart-item.dto';
import { UpdateCartItemDto } from '@/modules/cart/presentation/dto/update-cart-item.dto';
...
export class CartItemCommandService {
    ...
    async checkoutCart(identity: CartIdentity): Promise<void> {
        ...
        if (!cart)
            throw new NotFoundException('Không tìm thấy giỏ hàng active.');
